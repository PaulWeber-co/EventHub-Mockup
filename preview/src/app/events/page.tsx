...
export default function EventsPage() {
  ...
  const fetchEvents = (params: { search: string; category: string; date: string }) => {
    setLoading(true);
    let filtered = [...mockEvents];

    if (params.search) {
      const s = params.search.toLowerCase();
      filtered = filtered.filter(e => e.title.toLowerCase().includes(s) || e.location.toLowerCase().includes(s));
    }
